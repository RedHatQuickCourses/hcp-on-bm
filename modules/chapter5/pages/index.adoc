...
This lab shows how to create and access a hosted cluster using worker nodes available from an ACM infrastrcuture environment.
...
9. Using your preferred method (web console or CLI) inspect the `kube-system` namespace to see the konnectivity-agent and kube-api-server-proxy pods that connect cluster nodes to their hosted control plane on the hub cluster. 
