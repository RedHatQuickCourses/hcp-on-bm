...
This lab describes how configure the bare metal host with virtual networking and a helper services VM, by using an Ansible playbook provided on a git repository. It also configures the bare metal host for graphical access, so you can run a web browser and virt-manager on the following labs.

1. Review the architecture of the HCP on bare metal proof-of-concept architecture.
+
image::s3-fig-1.png[]
+
For now, you will configure just the libvirt network and the helper services VM.
+
NOTE: There is only one helper VM, which runs both DNS, HAProxy, and BOOTP daemons.
...
--
+
NOTE: If your SSH client supports it, enable keep alive so the connection doesn't terminate during long-running Ansible playbooks, killing the Ansible runner and leeting your host only partially configured.
+
[source,subs="verbatim,quotes"]
--
...
5. Configure remote graphicall access, using one of the two alternatives:

.. If your worktation runs Fedora Linux or any other Linux distribution with Wayland, you can use `waypipe` in front of the `ssh` command that connects to your EC2 instance, and start a web browser from the shell.
+
[source,subs="verbatim,quotes"]
--
...
.. If your workstation does not run Wayland, you can setup VNC access.
+
[source,subs="verbatim,quotes"]
--
...
.. Connect to the public ip of the baremetal node on port 5999 using `vncviewer` to get its graphical console and open a web browser.

.. The following instructions will ask you to access the Red Hat customer portal and Red Hat Hybrid Cloud Console. You can do it directly from the bare metal instance, using the graphicall access you just configured, or access those sites from your workstation and them copy files to the EC2 instance.

6. Download latest `rhel-9.5-x86_64-kvm.qcow2` cloud image (*Red Hat Enterprise Linux 9.5 KVM Guest Image*) from the https://access.redhat.com/downloads[customer portal] and copy to the baremetal node.
+
...
--
+
NOTE: We use a fork of the https://github.com/v2pkthakur/hcp-on-bm.git[original repository^] to ensure the reproducibility of the instructions here. The original repository may include contributitions and improvements, made after creation of this course, which may interest you.
...
13. Set up the baremetal node by running the ansible playbook.
+
[source,subs="verbatim,quotes"]
--
...
15. Allow access to `virt-manager` from your GUI user.
