...
_Watch the following video and them follow the instructions bellow._

Looks like video macros won't work with the Red Hat media center:

video::https://videos.learning.redhat.com/media/hcp-on-bm-bm-host/1_rtsirpp6[title="Prepare the bare metal node segment from the Red Hat One 2025 session – Maximizing ROI with Hosted Control Planes: Strategies for Scalable Environments"]

This attempt uses an inframe in a pass-though block:
